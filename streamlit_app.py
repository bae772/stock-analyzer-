...
st.title("베짱이표 AI주식 분석기 (미국주식 AI 분석 + 투자 전략)")
...
if ticker:
    try:
        ...
        open_s, high, low, close, volume = data['Open'], data['High'], data['Low'], data['Close'], data['Volume']
        ...
        # ───────────────────────────────────────────────
        # 에너지 패턴 계산 (Body/Shadow 비율 기반) 및 잔존률
        # ───────────────────────────────────────────────
        # candle body: abs(close - open), shadow total: high - low
        candle_body = (close - open_s).abs()
        shadow_total = (high - low).replace(0, np.nan)  # zero->nan to avoid div by zero
        energy = (candle_body / shadow_total).replace([np.inf, -np.inf], 0).fillna(0)

        # 최근 n일 평균 대비 잔존률 (안전 처리)
        n_residual = 10
        energy_mean_n = energy.rolling(n_residual).mean()
        energy_mean_last = energy_mean_n.iloc[-1] if not np.isnan(energy_mean_n.iloc[-1]) else 0.0
        # avoid division by zero
        energy_residual = 0.0
        if energy_mean_last > 0:
            energy_residual = round(float(energy.iloc[-1] / energy_mean_last * 100), 1)
        else:
            energy_residual = round(float(energy.iloc[-1] * 100), 1)  # fallback scaled

        # 에너지 패턴 텍스트 (최근 5일 평균과 비교)
        energy_mean_5 = energy.rolling(5).mean().iloc[-1] if not np.isnan(energy.rolling(5).mean().iloc[-1]) else 0.0
        energy_pattern = "에너지 중립"
        if energy.iloc[-1] > energy_mean_5 * 1.05:
            energy_pattern = "상승 에너지 강화"
        elif energy.iloc[-1] < energy_mean_5 * 0.95:
            energy_pattern = "하락 에너지 강화"

        # 간단 전환 체크 (최근 3캔들 흐름 + 에너지 증가)
        if len(close) >= 3:
            if (close.iloc[-3] < close.iloc[-2] < close.iloc[-1]) and (energy.iloc[-1] > energy.iloc[-2]):
                energy_pattern = "상승 에너지 전환"
            elif (close.iloc[-3] > close.iloc[-2] > close.iloc[-1]) and (energy.iloc[-1] > energy.iloc[-2]):
                energy_pattern = "하락 에너지 전환"

        # ───────────────────────────────────────────────
        # 복합 패턴 탐지 (BUY / SELL) — 복합 조건 만족 시 신호
        # ───────────────────────────────────────────────
        ...
        # ───────────────────────────────────────────────
        # Plotly 차트 (Close + MA20/50 + Bollinger) — NaN 처리 및 index reset
        # ───────────────────────────────────────────────
        chart_len = 250
