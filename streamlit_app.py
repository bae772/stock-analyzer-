...
from yahoofinancials import YahooFinancials  # 뉴스용 (pip install yahoofinancials)
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        for k,v in periods_for_calc.items():
            if len(close) > v:
                start_price = float(close.iloc[-v-1])
        ...
        # ── RSI, 이동평균, 거래량, MACD, 볼린저밴드, ATR 계산 ──
        delta = close.diff()
        ...
        if score>=80: grade,gcolor="A (강력매수)","#00ff00"
        elif score>=70: grade,gcolor="A (매수)","#33ff33"
        elif score>=60: grade,gcolor="B (관망)","#ffff33"
        elif score>=40: grade,gcolor="C (주의)","#ff9933"
        else: grade,gcolor="D (매도)","#ff3333"

        # ── 뉴스 가져오기 (Yahoo Financials) ──
        try:
            yf_fin = YahooFinancials(ticker)
            news_list = yf_fin.get_stock_news_data() or []
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown(f"**기간별 등락률:** " + ", ".join([f"{k}: {v}%" if v is not None else f"{k}: 데이터 부족" for k,v in period_change_pct.items()]))
            if avg_price>0:
                profit_pct = round((last_close-avg_price)/avg_price*100,2)
                st.markdown(f"**평단가 대비 수익률:** {profit_pct}%")
            st.markdown("**최근 뉴스:**")
            if headlines:
                for h in headlines:
                    st.markdown(f"- {h}")
