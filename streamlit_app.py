...
st.title("📊 간단 주식 분석 앱 (BB 제거 + 매수 판단 + 점수)")
...
if ticker:
    try:
        ...
        if data.empty:
            ...
            st.subheader("단순 매수/매도 판단")
            st.markdown(f"**{action}**")

            # --------------------
            # 점수 계산 (100점 기준)
            # 단순 계산: 최근 추세(종가 변화) 50%, RSI 적정도 50%
            recent_period = min(days, len(data))
            past_close = data['Close'].iloc[-recent_period]
            trend_score = (last_close / past_close) * 50
            rsi_score = max(0, 100 - abs(50 - last_rsi)) * 0.5
