...
# 사용자 입력
tickers = st.text_input("티커 입력 (쉼표로 구분, 예: BMR, TSLA, MARA)", value="BMR").upper()
avg_price = st.number_input("보유 평단가 입력 (없으면 0)", min_value=0.0, step=0.01, value=0.0)

# 기간 선택
period_option = st.selectbox("기간 선택", ["15년","10년","5년","1년","3개월","1주일","1일","실시간"])
today = datetime.today().date()
if period_option == "15년":
...
elif period_option == "1일":
    start_date = today - pd.DateOffset(days=1)
elif period_option == "실시간":
    start_date = today - pd.DateOffset(days=1)
end_date = today
...
for ticker in ticker_list:
    try:
        ...
        if data.empty or len(data) < 2:
            st.error(f"{ticker} 데이터가 부족하거나 티커를 찾을 수 없습니다.")
            continue
        ...
        last_date = close.index[-1].strftime('%Y-%m-%d %H:%M' if period_option=="실시간" else '%Y-%m-%d')

        # 선택 기간 대비 수익률
        start_price = float(close.iloc[0])
        period_change_pct = round((last_close - start_price)/start_price * 100, 2)
        ...
        <div style="background:#000; color:white; padding:30px; border-radius:20px; 
            ...
            <p style="color:#00ffff; font-size:1.4em; margin:5px;">
                선택 기간 대비: {period_change_pct}%
            </p>
            ...
            <p style="font-size:1.3em; margin:10px 0;">판매 안내: {sell_advice}</p>
