...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # 기간별 등락률 계산
        def calc_change(data, period_days=1):
            if len(data) < period_days+1:
                return None
            past_close = float(data['Close'].iloc[-period_days-1])
            pct = round((last_close - past_close)/past_close*100, 2)
            return pct

        change_dict = {}
        if selected_time in ["실시간","1일","1주"]:
            change_dict[selected_time] = round((last_close - float(close.iloc[-2]))/float(close.iloc[-2])*100,2)
        elif selected_time in ["1개월","3개월","6개월","1년","5년","10년","15년"]:
            period_days_map = {"1개월":21, "3개월":63, "6개월":126, "1년":252, "5년":252*5, "10년":252*10, "15년":252*15}
            pct = calc_change(data, period_days=period_days_map[selected_time])
            change_dict[selected_time] = pct
        ...
        # AI SCORE
        change_pct = change_dict[selected_time] if change_dict[selected_time] is not None else 0
        base_score = 50.0
        ...
        reason = (
            ...
            f"{selected_time} 전 대비 변화: {change_dict[selected_time]:.2f}%"
        )
