...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        macd_slope = float(macd_diff.diff().iloc[-4:].mean())
        ...
        upper_shadow = float(high.iloc[-1] - max(last_close, last_open))
        lower_shadow = float(min(last_close, last_open) - low.iloc[-1])
        if lower_shadow > body * 2: enhanced += 10  # 망치형
        if upper_shadow > body * 2: enhanced -= 10  # 역망치형
        if body > candle_range * 0.6 and last_close > last_open: enhanced += 8  # 장대양봉
        if body > candle_range * 0.6 and last_close < last_open: enhanced -= 8  # 장대음봉
        if body < candle_range * 0.1: enhanced -= 3  # 도지
        ...
        score = int(np.clip(score + enhanced, 0, 100))
