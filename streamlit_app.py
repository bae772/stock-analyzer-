...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 선택 기간 등락률 계산 ──
        period_days_map = {
            "실시간": 1, "1일": 1, "1주": 5, "1개월": 22, "3달": 66, "6달": 132,
            "1년": 252, "5년": 1260, "10년": 2520, "15년": 3780
        }
        days_back = period_days_map.get(selected_time, 1)
        if len(close) > days_back:
            past_close = float(close.iloc[-days_back])
            pct_change_period = round((last_close - past_close)/past_close*100,2)
            period_text = f"{selected_time} 기준 등락률: {pct_change_period}%"
        else:
            period_text = f"{selected_time} 기준 등락률: 데이터 부족"

        # ── RSI, 이동평균, 거래량, MACD, 볼린저밴드 등 계산 ──
        delta = close.diff()
        ...
        if score>=80: grade="A (강력매수)"; reason="RSI 저평가 + 상승추세 + MACD 골든크로스 등 긍정적 신호"
        elif score>=70: grade="A (매수)"; reason="추세 상승 + 일부 기술적 지표 긍정적"
        elif score>=60: grade="B (관망)"; reason="단기 변동성 존재, 신중 관망 필요"
        elif score>=40: grade="C (주의)"; reason="과열/하락 위험, 일부 매수 가능성만"
        else: grade="D (매도)"; reason="과열/하락 신호 다수, 매수 지양"
