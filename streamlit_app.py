...
if ticker:
    try:
        ...
        score = 50
        if last_rsi < 50:
            score += (30 - last_rsi) * 1.2
        else:
            score -= (last_rsi - 50) * 0.8

        score += change_pct * 3

        if vol_ratio > 1:
            score += min(vol_ratio - 1, 5) * 8
        else:
            score -= 10
        ...
        # 등급
        if score >= 80:
            grade, grade_color = "A (강력매수)", "#00ff00"
        elif score >= 70:
            grade, grade_color = "A (매수)", "#33ff33"
        elif score >= 60:
            grade, grade_color = "B (관망)", "#ffff33"
        elif score >= 40:
            grade, grade_color = "C (주의)", "#ff9933"
        else:
            grade, grade_color = "D (매도)", "#ff3333"
