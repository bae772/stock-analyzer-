...
# ── 뉴스 가져오기 함수 (예시) ──
def fetch_news_for_ticker(ticker, api_key="YOUR_API_KEY", max_articles=3):
    endpoint = f"https://stocknewsapi.com/api/v1?tickers={ticker}&items={max_articles}&token={api_key}"
    articles = []
    try:
        ...
        if resp.status_code == 200:
            ...
            for art in data:
                title = art.get("title")
                url = art.get("url")
                summary = art.get("text", "")[:200]  # 간단 요약
                sentiment = art.get("sentiment", 0)
                label = "호재" if sentiment > 0 else ("악재" if sentiment < 0 else "중립")
                articles.append({
                    "title": title,
                    "url": url,
                    "summary": summary,
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        period_change_pct = {}
        periods_for_calc = {
            "1일": 1, "1주": 5, "1달": 22, "3달": 66, "6달": 132, "1년": 252
        }
        for k,v in periods_for_calc.items():
            if len(close) > v:
                pct = round((close.iloc[-1] - close.iloc[-v-1])/close.iloc[-v-1]*100,2)
                period_change_pct[k] = pct
            else:
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            # 기간별 등락률 표시
            st.markdown("### 📊 기간별 등락률")
            for period_label, pct in period_change_pct.items():
                if pct is not None:
                    color = "#00ff00" if pct >= 0 else "#ff3333"
                    st.markdown(f"- **{period_label}** : <span style='color:{color};'>{pct}%</span>", unsafe_allow_html=True)
                else:
                    st.markdown(f"- **{period_label}** : 데이터 없음")

            # 뉴스
            news_list = fetch_news_for_ticker(ticker, api_key="YOUR_API_KEY")
            if news_list:
                st.markdown("#### 📰 최근 뉴스")
                for n in news_list:
                    st.markdown(f"- **[{n['title']}]({n['url']})** — {n['label']}")
