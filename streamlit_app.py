...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # 기간별 등락률 계산
        def calc_pct(start_offset):
            if len(close) > start_offset:
                pct = (close.iloc[-1] - close.iloc[-start_offset-1])/close.iloc[-start_offset-1]*100
                return round(pct,2)
            else:
                return None
        ...
        percent_changes = {
            "실시간": calc_pct(1),
            "1일": calc_pct(1),
            "1주": calc_pct(5),   # 대략 5거래일
            "1달": calc_pct(21),  # 약 21거래일
            "3달": calc_pct(63),  # 약 63거래일
            "6달": calc_pct(126),
            "1년": calc_pct(252),
            "5년": calc_pct(252*5),
            "10년": calc_pct(252*10),
        ...
        percent_text = {k: f"{v}%" if v is not None else "데이터 부족" for k,v in percent_changes.items()}
        ...
        if percent_changes[selected_time] is not None:
            base_score += percent_changes[selected_time]*2.0
        ...
        # 등급/근거
        if score>=80: grade,gcolor="A (강력매수)","#00ff00"; reason="RSI 저평가 + 상승추세 + MACD 골든크로스 등 긍정적 신호"
        elif score>=70: grade,gcolor="A (매수)","#33ff33"; reason="추세 상승 + 일부 기술적 지표 긍정적"
        elif score>=60: grade,gcolor="B (관망)","#ffff33"; reason="단기 변동성 존재, 신중 관망 필요"
        elif score>=40: grade,gcolor="C (주의)","#ff9933"; reason="과열/하락 위험, 일부 매수 가능성만"
        else: grade,gcolor="D (매도)","#ff3333"; reason="과열/하락 신호 다수, 매수 지양"
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown(f"**기간별 등락률:** {', '.join([f'{k}: {v}' for k,v in percent_text.items()])}")
