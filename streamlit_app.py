...
# ── 퍼센트 계산 함수 ──
def pct_change_from_days(df_close, days):
    df_close = df_close.dropna()
    if len(df_close) < 2:
        return None
    target_date = df_close.index[-1] - pd.Timedelta(days=days)
    past_price_series = df_close[df_close.index <= target_date]
    if past_price_series.empty:
        past_price = float(df_close.iloc[0])
    else:
        past_price = float(past_price_series.iloc[-1])
    recent_price = float(df_close.iloc[-1])
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # 기간별 상승률 계산
        period_changes = {}
        periods_days = {"1일":1,"1주":7,"1달":30,"3달":90,"6달":180,"1년":365}
        for p_name, p_days in periods_days.items():
            pct = pct_change_from_days(close, p_days)
            if pct is not None:
                period_changes[p_name] = f"{pct:+.2f}%"
        ...
        base_score += 0 # 단순화: 기간별 상승률 반영 가능
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown("**기간별 등락률:**")
            for k,v in period_changes.items():
                st.markdown(f"- {k}: {v}")
