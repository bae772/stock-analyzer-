...
# ── 안전하게 마지막 값 가져오기 함수 ──
def safe_last_val(series, default=0.0):
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        last_close = safe_last_val(close)
        prev_close = safe_last_val(close[:-1])
        ...
        rsi = 100 - 100/(1+rs)
        last_rsi = safe_last_val(rsi)

        # 이동평균
        ma20 = close.rolling(20).mean()
        ma50 = close.rolling(50).mean()
        last_ma20 = safe_last_val(ma20)
        last_ma50 = safe_last_val(ma50)

        # 거래량
        vol_ma20 = volume.rolling(20).mean()
        last_vol_ma20 = safe_last_val(vol_ma20, default=1.0)
        vol_today = safe_last_val(volume)
        ...
        last_macd_diff = safe_last_val(macd_diff)

        # 볼린저밴드
        std20 = close.rolling(20).std()
        upper_band = ma20 + 2*std20
        lower_band = ma20 - 2*std20
        last_upper = safe_last_val(upper_band)
        last_lower = safe_last_val(lower_band)
        ...
        last_atr = safe_last_val(atr, default=0.0)
        ...
        # 1) RSI 구간 세분화
        if last_rsi < 10: enhanced += 15
        elif last_rsi < 20: enhanced += 10
        elif last_rsi < 30: enhanced += 6
        elif last_rsi < 40: enhanced += 3
        elif last_rsi > 85: enhanced -= 15
        elif last_rsi > 75: enhanced -= 10
        elif last_rsi > 65: enhanced -= 6

        # 2) MACD 세부 분석
        macd_slope = macd_diff.diff().iloc[-4:].mean() if len(macd_diff) >= 4 else 0.0
        macd_hist_strength = abs(last_macd_diff)

        if macd_slope > 0: enhanced += 8
        else: enhanced -= 7

        if macd_hist_strength > 0.5: enhanced += 8
        elif macd_hist_strength < 0.1: enhanced -= 4

        # 3) 거래량 상태
        if vol_ratio >= 4: enhanced += 15
        elif vol_ratio >= 3: enhanced += 10
        elif vol_ratio >= 2: enhanced += 6
        elif vol_ratio >= 1.2: enhanced += 3
        elif vol_ratio < 0.7: enhanced -= 7
        elif vol_ratio < 0.5: enhanced -= 10

        # 4) 이동평균 배열
        if last_ma20 > last_ma50: enhanced += 7
        else: enhanced -= 6

        # 5) 가격 / 볼린저 위치 점수
        if last_close < last_lower: enhanced += 12
        if last_close > last_upper: enhanced -= 12

        # 6) 캔들 패턴
        last_open = safe_last_val(close.shift(1))
        body = abs(last_close - last_open)
        candle_range = safe_last_val(high - low, default=0.0)
        upper_shadow = safe_last_val(high - np.maximum(last_close, last_open), default=0.0)
        lower_shadow = safe_last_val(np.minimum(last_close, last_open) - low, default=0.0)

        if lower_shadow > body * 2: enhanced += 10
        if upper_shadow > body * 2: enhanced -= 10
        if body > candle_range * 0.6 and last_close > last_open: enhanced += 8
        if body > candle_range * 0.6 and last_close < last_open: enhanced -= 8
        if body < candle_range * 0.1: enhanced -= 3

        # 7) 변동성(ATR)
        atr_ratio = last_atr / last_close if last_close != 0 else 0
        if atr_ratio > 0.07: enhanced += 10
        elif atr_ratio > 0.05: enhanced += 5
        elif atr_ratio < 0.02: enhanced -= 4

        # 8) 단기추세
        recent_trend = close.diff().iloc[-5:].mean() if len(close) >= 5 else 0
        if recent_trend > 0: enhanced += 6
        else: enhanced -= 6

        # 9) 고점/저점 돌파
        highest_20 = safe_last_val(high[-20:].max())
        lowest_20 = safe_last_val(low[-20:].min())
        if last_close >= highest_20 * 0.995: enhanced += 12
        if last_close <= lowest_20 * 1.005: enhanced -= 12

        # 10) 다이버전스
        price_diff = safe_last_val(close.iloc[-1] - close.iloc[-3]) if len(close) >= 3 else 0
        macd_diff3 = safe_last_val(macd_diff.iloc[-1] - macd_diff.iloc[-3]) if len(macd_diff) >= 3 else 0

        if price_diff < 0 and macd_diff3 > 0: enhanced += 12
        if price_diff > 0 and macd_diff3 < 0: enhanced -= 12

        # 11) 거래량 + 차트 결합 패턴
        if vol_ratio > 2 and last_close > last_open: enhanced += 12
        if vol_ratio > 2 and last_close < last_open: enhanced -= 12

        # 12) 장중 변동폭 점수
        intraday_range = safe_last_val(high - low)
        if intraday_range > last_atr * 1.7: enhanced += 6

        # 최종 점수
        score = int(np.clip(base_score + enhanced, 0, 100))
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown(f"**판매 안내:** {sell_advice}")
