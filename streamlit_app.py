...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        target_price = round(max(last_close + last_atr*2.5, high_20*1.03), 2)
        stop_price = round(min(last_close - last_atr*1.8, low_20*0.97), 2)
        ...
        if avg_price>0:
            ...
            if last_close>=target_price:
                sell_advice_text="목표가 도달! 매도 고려"
            elif last_rsi>70:
                sell_advice_text="RSI 과열, 단기 매도 가능"
            elif last_close<stop_price:
                sell_advice_text="손절가 도달, 손절 권장"
            else:
                sell_advice_text=f"목표가: {target_price} USD, 손절가: {stop_price} USD"
        else:
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown(f"**목표가(Target Price):** {target_price} USD")
            st.markdown(f"**손절가(Stop Loss):** {stop_price} USD")
            st.markdown(f"**판매 안내:** {sell_advice_text}")
