...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 기간별 등락률 계산 (토스증권처럼 정확하게)
        pct_changes = {}
        period_offsets = {
            "1일": pd.Timedelta(days=1),
            "1주": pd.Timedelta(weeks=1),
            "1개월": pd.DateOffset(months=1),
            "3달": pd.DateOffset(months=3),
            "6달": pd.DateOffset(months=6),
            "1년": pd.DateOffset(years=1)
        }

        for period_name, offset in period_offsets.items():
            target_date = close.index[-1] - offset
            past_close_series = close[close.index <= target_date]
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            for p_name, pct in pct_changes.items():
                if pct is not None:
                    up_down = "상승" if pct > 0 else "하락" if pct < 0 else "변동 없음"
                    st.markdown(f"- {p_name} 전 대비: {abs(pct)}% {up_down}")
