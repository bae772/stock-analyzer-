...
# ── 기간별 퍼센트 계산 함수 ──
def calc_pct_change(df_close, days):
    if len(df_close) < days+1:
        return None
    past_price = float(df_close.dropna().iloc[-(days+1)])
    recent_price = float(df_close.dropna().iloc[-1])
    return round((recent_price - past_price) / past_price * 100, 2)
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        last_ma50 = float(ma50.dropna().iloc[-1])

        # 거래량
        vol_ma20 = volume.rolling(20).mean()
        last_vol_ma20 = float(vol_ma20.dropna().iloc[-1])
        vol_today = float(volume.dropna().iloc[-1])
        ...
        last_macd_diff = float(macd_diff.dropna().iloc[-1])

        # 볼린저밴드
        std20 = close.rolling(20).std()
        upper_band = ma20 + 2*std20
        lower_band = ma20 - 2*std20
        last_upper = float(upper_band.dropna().iloc[-1])
        last_lower = float(lower_band.dropna().iloc[-1])
        ...
        if score>=80:
            grade,gcolor="A (강력매수)","#00ff00"; reason="RSI 저평가 + 상승추세 + MACD 골든크로스"
        elif score>=70:
            grade,gcolor="A (매수)","#33ff33"; reason="추세 상승 + 일부 기술적 지표 긍정적"
        elif score>=60:
            grade,gcolor="B (관망)","#ffff33"; reason="단기 변동성 존재, 신중 관망 필요"
        elif score>=40:
            grade,gcolor="C (주의)","#ff9933"; reason="과열/하락 위험, 일부 매수 가능성만"
        else:
            grade,gcolor="D (매도)","#ff3333"; reason="과열/하락 신호 다수, 매수 지양"
        ...
        # ── 기간별 퍼센트 계산
        pct_1d = calc_pct_change(close,1)
        pct_1w = calc_pct_change(close,5)
        pct_1mo = calc_pct_change(close,21)
        pct_3mo = calc_pct_change(close,63)
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown(f"**기간별 변동률:** 1일: {pct_1d if pct_1d is not None else 'N/A'}% | "
                        f"1주: {pct_1w if pct_1w is not None else 'N/A'}% | "
                        f"1달: {pct_1mo if pct_1mo is not None else 'N/A'}% | "
                        f"3달: {pct_3mo if pct_3mo is not None else 'N/A'}%")
