...
start_date = st.date_input("시작일", value=date.today().replace(year=date.today().year-1))
...
if ticker:
    try:
        ...
        if data.empty or len(data) < 50:
            st.error("선택한 기간의 데이터가 부족하거나 티커를 찾을 수 없습니다.")
            st.stop()
        ...
        # ── Plotly 차트 ──
        chart_close = close.reset_index(drop=True)
        chart_ma20 = ma20.reset_index(drop=True)
        chart_ma50 = ma50.reset_index(drop=True)
        chart_upper = upper_band.reset_index(drop=True)
        chart_lower = lower_band.reset_index(drop=True)
        x_axis = data.index  # 실제 날짜
