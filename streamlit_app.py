...
if ticker:
    try:
        ...
        if score >= 80:
            ...
            strategy = "적극적 매수 추천, 단기 변동 대비 일부 분할 매수 가능"
        elif score >= 70:
            interpretation = "매수: 상승 가능성이 있으나 주의 필요"
            strategy = "일부 매수 가능, 추세 확인 후 분할 매수"
        elif score >= 60:
            interpretation = "관망: 단기 반등 가능, 신중한 접근 권장"
            strategy = "관망 위주, 변동성 큰 구간에서는 소량만"
        elif score >= 40:
            interpretation = "주의: 투자 리스크 높음, 소량/관망 추천"
            strategy = "매수보다는 관망, 단기 하락 대비 손절가 설정"
        else:
            interpretation = "매도: 현재 매수보다는 회피 권장, 하락 위험 높음"
            strategy = "매수 지양, 이미 보유 시 손절가 엄격히 관리"
        ...
        <div style="background:#111; color:white; padding:20px; border-radius:18px; margin:20px 0;
            ...
            <p style="font-size:1.2em; margin:5px 0; font-weight:bold;">추천 전략: {strategy}</p>
        </div>
        """, unsafe_allow_html=True)
