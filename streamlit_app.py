...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 기간별 등락률 계산 (토스증권 기준 정확) ──
        def get_close_n_days_ago(close_series, n_days):
            target_date = close_series.index[-1] - BDay(n_days)
            closest_idx = close_series.index.get_indexer([target_date], method='nearest')[0]
            return float(close_series.iloc[closest_idx])
        ...
        for period_name, days_back in period_dict.items():
            if len(close) > days_back:
                past_close = get_close_n_days_ago(close, days_back)
                pct = round((last_close - past_close)/past_close*100,2)
                if pct>=0:
                    pct_changes[period_name] = f"{period_name} 전 대비 {pct}% 상승"
                else:
                    pct_changes[period_name] = f"{period_name} 전 대비 {abs(pct)}% 하락"
            else:
                pct_changes[period_name] = f"{period_name} 전 대비 데이터 부족"
        ...
        base_score += 0  # change_pct 제거, 등락률은 따로 표시
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            # ── 기간별 등락률 표시
            st.markdown("**기간별 등락률:**")
            for pct_text in pct_changes.values():
