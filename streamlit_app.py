...
# ── 사용자 입력 ──
tickers_input = st.text_input("티커 입력 (쉼표로 여러개 가능, 예: BMR, TSLA, MARA)", value="BMR").upper()
...
selected_time = st.selectbox("기간 선택", list(time_options.keys()))
...
for ticker in tickers:
    try:
        ...
        if data.empty or len(data)<10:
            st.warning(f"{ticker} 데이터가 부족합니다.")
            continue
