...
# ── 기간별 등락률 계산 함수 ──
def get_pct_change(close, days):
    try:
        if len(close) <= days:
            return None
        past_close = float(close.iloc[-days-1])
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 기간별 등락률 계산
        pct_changes = {}
        for name, days in period_days_map.items():
            pct = get_pct_change(close, days)
        ...
        if score>=80:
            ...
            reason=f"RSI: {last_rsi:.2f} (저평가) + 상승추세 + MACD 골든크로스 등 긍정적 신호"
        elif score>=70:
            grade,gcolor="A (매수)","#33ff33"
            reason=f"RSI: {last_rsi:.2f} + 추세 상승 및 기술적 지표 긍정적"
        elif score>=60:
            grade,gcolor="B (관망)","#ffff33"
            reason=f"RSI: {last_rsi:.2f} + 단기 변동성 존재, 신중 관망 필요"
        elif score>=40:
            grade,gcolor="C (주의)","#ff9933"
            reason=f"RSI: {last_rsi:.2f} + 과열/하락 위험, 일부 매수 가능성만"
        else:
            grade,gcolor="D (매도)","#ff3333"
            reason=f"RSI: {last_rsi:.2f} + 과열/하락 신호 다수, 매수 지양"
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            for p_name, pct in pct_changes.items():
                if pct is not None:
                    if pct >= 0:
                        st.markdown(f"- {p_name} 전 대비: {pct:.2f}% 상승")
                    else:
                        st.markdown(f"- {p_name} 전 대비: {abs(pct):.2f}% 하락")
