...
# ── 사용자 입력 ──
tickers_input = st.text_input("티커 입력 (쉼표로 구분, 예: BMR,TSLA,MARA)", value="TSLA")
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 기간별 등락률
        period_dict = {
            "1일": 1, "1주": 5, "1달": 22, "3달": 66, "6달": 132, "1년": 252, "5년": 1260, "10년": 2520, "15년": 3780
        }
        if selected_time in period_dict and len(close) > period_dict[selected_time]:
            past_close = float(close.iloc[-period_dict[selected_time]])
            period_change_pct = round((last_close - past_close)/past_close*100,2)
        else:
        ...
        if score>=80:
            ...
            reason="RSI 저평가, 상승 추세, MACD 골든크로스 등 긍정적 신호"
        elif score>=70:
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown(f"**기간별 변동률:** {period_change_pct}%")
