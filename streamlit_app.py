...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # 6) 캔들 패턴 세분화 
        last_open = float(data['Close'].shift(1).iloc[-1])
        ...
        if price_diff < 0 and macd_diff3 > 0:
            enhanced += 12
        if price_diff > 0 and macd_diff3 < 0:
            enhanced -= 12

        # 11) 거래량 + 차트 결합 패턴
        if vol_ratio > 2 and last_close > last_open:
            enhanced += 12  # 강한 매수 수급
        if vol_ratio > 2 and last_close < last_open:
            enhanced -= 12  # 강한 매도 수급

        # 12) 장중 변동폭 점수
        intraday_range = high.iloc[-1] - low.iloc[-1]
        if intraday_range > last_atr * 1.7:
            enhanced += 6
