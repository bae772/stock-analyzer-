...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 1일 / 1주 / 3달 전 대비 퍼센트
        pct_changes = {}
        periods = {"1일":1, "1주":5, "3달":63}  # 거래일 기준
        ...
        last_ma20 = float(ma20.iloc[-1]) if not np.isnan(ma20.iloc[-1]) else last_close
        last_ma50 = float(ma50.iloc[-1]) if not np.isnan(ma50.iloc[-1]) else last_close

        # ── 거래량
        vol_ma20 = volume.rolling(20).mean()
        last_vol_ma20 = float(vol_ma20.iloc[-1]) if not np.isnan(vol_ma20.iloc[-1]) else 1.0
        vol_today = float(volume.iloc[-1])
        vol_ratio = round(vol_today / last_vol_ma20,2)
