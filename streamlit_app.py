...
st.set_page_config(page_title="베짱이 계산기 실시간", layout="centered")
st.title("베짱이 계산기 (미국주식 실시간 + AI 분석)")
...
# 실시간/날짜 선택
realtime = st.checkbox("실시간 데이터 보기 (1분 단위)", value=True)
start_date = st.date_input("시작일 (실시간 체크 시 무시됨)", value=datetime.now() - timedelta(days=7))
end_date = st.date_input("종료일 (실시간 체크 시 무시됨)", value=datetime.now())
...
if ticker:
    try:
        if realtime:
            ...
            if data.empty:
                st.warning("실시간 데이터가 없습니다. 최근 거래일 1일 데이터를 사용합니다.")
                today = datetime.now().date()
                data = yf.download(ticker, start=today, end=today + timedelta(days=1), interval="1m",
        ...
        if data.empty or len(data) < 20:
            st.error("데이터가 부족합니다. 티커 또는 기간을 확인하세요.")
            st.stop()
        ...
        if last_rsi < 30 and last_macd_diff > 0 and last_close < last_lower:
            Buy_Signal = True
            Sell_Signal = False
            signal_reason = "강력 매수: RSI 매우 저평가 + MACD 골든크로스 + 볼린저밴드 하단"
            ...
            Buy_Signal = False
            Sell_Signal = True
            signal_reason = "매도: RSI 과열 + 볼린저밴드 상단 돌파"
        else:
        ...
        fig.update_layout(title=f"{ticker} 실시간 차트", xaxis_title="기간", yaxis_title="가격", template='plotly_dark')
        st.plotly_chart(fig, use_container_width=True)

        st.caption(f"업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M')} │ 데이터: Yahoo Finance (실시간 1분)")
