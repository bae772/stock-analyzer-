...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 기간별 등락률 (1일, 1주, 3달)
        pct_changes = {}
        periods_back = {"1일":1, "1주":5, "3달":66}  # 거래일 기준
        for p_name, back_days in periods_back.items():
            if len(close) > back_days:
                past_close = float(close.iloc[-back_days])
                pct_changes[p_name] = round((last_close - past_close)/past_close*100,2)
            else:
                pct_changes[p_name] = None
        ...
        change_pct_recent = pct_changes.get("1일",0)
        base_score += change_pct_recent*2.0*score_weights.get("BUY",1.0)
        ...
        if last_rsi<30 and last_macd_diff>0 and last_close<last_lower:
            ...
            Buy_Signal=False; Sell_Signal=False; signal_reason="관망"
