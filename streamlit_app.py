...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        try:
            ...
            # 1일 전
            one_day_idx = len(close) - 2
            if one_day_idx >= 0:
                pct_changes['1일'] = round((last_close - close.iloc[one_day_idx])/close.iloc[one_day_idx]*100,2)

            # 1주 전 (5 영업일)
            one_week_idx = len(close) - 6
            if one_week_idx >= 0:
                pct_changes['1주'] = round((last_close - close.iloc[one_week_idx])/close.iloc[one_week_idx]*100,2)

            # 3달 전 (약 66 영업일)
            three_month_idx = len(close) - 67
            if three_month_idx >= 0:
                pct_changes['3달'] = round((last_close - close.iloc[three_month_idx])/close.iloc[three_month_idx]*100,2)
        ...
        vol_ratio = round(vol_today / float(vol_ma20.iloc[-1]),2) if vol_ma20.iloc[-1]>0 else 1.0
