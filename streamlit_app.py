...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        try:
            if len(close) > 1:
                past_close_1d = float(close.iloc[-2])
                pct_changes['1일'] = round((last_close - past_close_1d)/past_close_1d*100,2)
            if len(close) > 5:
                past_close_1w = float(close.iloc[-6])
                pct_changes['1주'] = round((last_close - past_close_1w)/past_close_1w*100,2)
            if len(close) > 66:
                past_close_3m = float(close.iloc[-67])
                pct_changes['3달'] = round((last_close - past_close_3m)/past_close_3m*100,2)
