...
for idx, ticker in enumerate(tickers):
    ...
    try:
        # ── 충분히 긴 기간 데이터 다운로드 (20년)
        data = yf.download(ticker, period="20y", interval="1d", progress=False, auto_adjust=True)
        ...
        base_score += 0  # change_pct 제거 (퍼센트는 따로 계산)
        ...
        if score>=80:
            ...
            reason="RSI 저평가, 상승추세 확인, MACD 골든크로스 등 긍정적 신호"
        elif score>=70:
            grade,gcolor="A (매수)","#33ff33"
            reason="주가 상승 추세 유지, 일부 기술적 지표 긍정적"
        elif score>=60:
            grade,gcolor="B (관망)","#ffff33"
            reason="단기 변동성 존재, 신중 관망 필요"
        elif score>=40:
            grade,gcolor="C (주의)","#ff9933"
            reason="과열/하락 위험 존재, 일부 매수 가능성만"
        else:
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            for p_name, pct in pct_changes.items():
                if pct is not None:
                    st.markdown(f"- {p_name} 전 대비: {pct}% 상승/하락")
