...
# ── 기간 선택 ──
time_options = ["실시간","1일","1주","1개월","3달","6달","1년","5년","10년","15년"]
...
# ── 장기 퍼센트 계산용 함수 ──
def get_long_term_pct(ticker, last_close):
    """
    6달 ~ 15년 대비 등락률 계산
    """
    try:
        long_data = yf.download(ticker, period="max", interval="1d", progress=False, auto_adjust=True)
        long_data = long_data['Close'].dropna()
        pct_changes_long = {}
        last_date = long_data.index[-1]
        periods = {
            "6달": timedelta(days=182),
            "1년": timedelta(days=365),
            "5년": timedelta(days=365*5),
            "10년": timedelta(days=365*10),
            "15년": timedelta(days=365*15)
        }
        for name, delta in periods.items():
            target_date = last_date - delta
            past_dates = long_data.index[long_data.index <= target_date]
            if len(past_dates) > 0:
                past_close = long_data.loc[past_dates[-1]]
                pct = round((last_close - past_close)/past_close*100,2)
                pct_changes_long[name] = pct
            else:
                pct_changes_long[name] = None
        return pct_changes_long
    except:
        return {k: None for k in ["6달","1년","5년","10년","15년"]}
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        for period_name, days_back in period_dict_short.items():
            if len(close) > days_back:
                past_close = float(close.iloc[-days_back])
                pct_changes[period_name] = round((last_close - past_close)/past_close*100,2)
            else:
                pct_changes[period_name] = None

        # ── 장기 등락률 계산
        pct_changes.update(get_long_term_pct(ticker, last_close))
        ...
        # ── AI SCORE
        base_score = 50.0
        change_pct = pct_changes.get("1일", 0)  # 오늘 대비 변화 사용
