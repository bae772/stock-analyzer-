...
if ticker:
    try:
        ...
        if data.empty:
            ...
            st.markdown(f"- **추세:** {'상승' if trend_score>0 else '하락'}")
            st.markdown(f"- **에너지:** RSI={data['RSI'].iloc[-1]:.1f}")
            st.markdown(f"- **패턴:** MA20={'상향' if data['MA20'].iloc[-1] > data['MA20'].iloc[-2] else '하향'}")
