...
with col1:
    start_date = st.date_input("시작일", value=datetime.now() - pd.Timedelta(days=180))
with col2:
...
if ticker:
    try:
        ...
        # 데이터 부족 시 자동 보정
        if len(data) < 50:
            st.warning("선택한 기간의 데이터가 부족하여 자동으로 1년치 데이터 사용 후 필터링합니다.")
            data = yf.download(ticker, period="1y", progress=False, auto_adjust=True)
            data = data[(data.index >= pd.to_datetime(start_date)) & (data.index <= pd.to_datetime(end_date))]

        if data.empty or len(data) < 50:
            st.error("데이터가 여전히 부족하거나 티커를 찾을 수 없습니다.")
            st.stop()
