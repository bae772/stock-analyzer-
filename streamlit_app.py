...
# ── 사용자 입력 ──
tickers_input = st.text_input("티커 입력 (쉼표로 구분, 예: TSLA,BMR)", value="TSLA")
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        period_change = get_change_pct(data, periods_days.get(selected_time,1))

        # AI SCORE
        base_score = 50.0
        base_score += max(0,30-last_rsi)*1.4*score_weights.get("BUY",1.0)
        base_score += period_change*0.5*score_weights.get("BUY",1.0)  # 변동률도 반영
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown(f"**기간 변동률:** {period_change:.2f}%")
            st.markdown(f"**AI Score:** {score} [{grade}]")
            st.markdown(f"**매수/매도 근거:**\n- RSI: {last_rsi:.2f}\n- MACD diff: {last_macd_diff:.2f}\n- 종가>MA20: {last_close>last_ma20}\n- 종가>MA50: {last_close>last_ma50}\n- 볼린저 밴드: 상단 {last_upper:.2f}/하단 {last_lower:.2f}\n- 거래량 대비: {vol_ratio}배")
