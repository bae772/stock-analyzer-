...
if ticker:
    try:
        ...
        # ── Signal 컬럼 생성 (bool 단일값) ──
        Buy_Signal = (last_rsi < 40) and (last_macd_diff > 0) and (change_pct >= -2)
        Sell_Signal = (last_rsi > 70) or (last_close > last_upper)

        # Signal Reason
        if Buy_Signal:
            signal_reason = "단기 매수 타이밍 (RSI 저평가 + MACD 골든크로스)"
        elif Sell_Signal:
            signal_reason = "단기 매도 타이밍 (과열 구간)"
        else:
            signal_reason = "관망"
