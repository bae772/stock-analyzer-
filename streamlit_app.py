...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # 2) MACD 세부
        macd_slope = macd_diff.diff().iloc[-4:].mean()
        macd_hist_strength = abs(last_macd_diff)
        enhanced += 8 if macd_slope>0 else -7
        enhanced += 8 if macd_hist_strength>0.5 else -4 if macd_hist_strength<0.1 else 0
        ...
        # 4) 이동평균 배열
        enhanced += 7 if last_ma20 > last_ma50 else -6

        # 5) 가격/볼린저 위치
        enhanced += 12 if last_close < last_lower else 0
        enhanced -= 12 if last_close > last_upper else 0

        # 6) 캔들패턴
        last_open = float(close.shift(1).iloc[-1])
        last_close_val = last_close  # 확실히 단일 값
        body = abs(last_close_val - last_open)
        candle_range = float(high.iloc[-1] - low.iloc[-1])
        upper_shadow = float(high.iloc[-1] - max(last_close_val, last_open))
        lower_shadow = float(min(last_close_val, last_open) - low.iloc[-1])
        if lower_shadow > body*2: enhanced += 10
        if upper_shadow > body*2: enhanced -= 10
        if body > candle_range*0.6 and last_close_val > last_open: enhanced += 8
        if body > candle_range*0.6 and last_close_val < last_open: enhanced -= 8
        if body < candle_range*0.1: enhanced -= 3

        # 7) 변동성 (ATR)
        atr_ratio = last_atr / last_close_val
        enhanced += 10 if atr_ratio>0.07 else 5 if atr_ratio>0.05 else -4 if atr_ratio<0.02 else 0

        # 8) 단기추세
        recent_trend = close.diff().iloc[-5:].mean()
        enhanced += 6 if recent_trend>0 else -6

        # 9) 고점/저점 돌파
        highest_20 = float(high[-20:].max())
        lowest_20 = float(low[-20:].min())
        enhanced += 12 if last_close_val >= highest_20*0.995 else 0
        enhanced -= 12 if last_close_val <= lowest_20*1.005 else 0

        # 10) 다이버전스
        price_diff = close.iloc[-1] - close.iloc[-3]
        macd_diff3 = macd_diff.iloc[-1] - macd_diff.iloc[-3]
        enhanced += 12 if price_diff<0 and macd_diff3>0 else 0
        enhanced -= 12 if price_diff>0 and macd_diff3<0 else 0

        # 11) 거래량+차트 결합
        enhanced += 12 if vol_ratio>2 and last_close_val>last_open else 0
        enhanced -= 12 if vol_ratio>2 and last_close_val<last_open else 0

        # 12) 장중 변동폭
        intraday_range = high.iloc[-1]-low.iloc[-1]
        enhanced += 6 if intraday_range > last_atr*1.7 else 0

        # 최종 AI 점수
        score = int(np.clip(base_score + enhanced,0,100))

        # ── Signal/등급/전략 계산
        if score>=80: grade,gcolor,reason="A (강력매수)","#00ff00","RSI 저평가 + 상승추세 + MACD 골든크로스 등 긍정적 신호"
