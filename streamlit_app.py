...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 기간별 등락률 (1일, 1주, 3개월) ──
        periods_back = {'1일':1, '1주':5, '3달':66}  # 영업일 기준
        pct_changes = {}
        for name, back in periods_back.items():
            if len(close) > back:
                past_close = float(close.iloc[-back-1])
                pct_changes[name] = round((last_close - past_close)/past_close*100,2)
            else:
