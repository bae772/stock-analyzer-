...
# ── 사용자 입력 ──
tickers_input = st.text_input("티커 입력 (쉼표로 구분, 예: TSLA, AAPL, BMR)", value="TSLA")
...
# ── 기간별 등락률 계산 함수 ──
def get_change_pct(data, period_label):
    ...
    if period_label in period_map_days:
        days = period_map_days[period_label]
        if len(data) > days:
    ...
    # 단기 데이터
    if len(data) >= 2:
        pct = (last_close - data['Close'].iloc[-2]) / data['Close'].iloc[-2] * 100
        return round(pct, 2)
    return 0.0

# ── 매수/매도 근거 포맷팅 함수 ──
def format_reason(last_rsi, last_macd_diff, last_close, ma20, ma50, upper, lower, vol_ratio, period_change):
    return (
        ...
        f"**종가 > MA20:** {'예' if last_close>ma20 else '아니오'}  |  **종가 > MA50:** {'예' if last_close>ma50 else '아니오'}  \n"
        f"**볼린저 밴드:** 상단 {upper:.2f} / 하단 {lower:.2f}  |  **종가:** {last_close:.2f}  \n"
        f"**거래량:** 평소 대비 {vol_ratio:.2f}배  \n"
        f"**선택 기간 대비 변화:** {period_change:.2f}%"
    )
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # 기간별 변화
        period_change = get_change_pct(data, selected_time)
        ...
        reason = format_reason(last_rsi, last_macd_diff, last_close, last_ma20, last_ma50, last_upper, last_lower, vol_ratio, period_change)
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            st.markdown(f"**선택 기간({selected_time}) 등락률:** {period_change:.2f}%")

            # Plotly 차트
            chart_len = 250
            chart_close = data['Close'][-chart_len:].reset_index(drop=True)
