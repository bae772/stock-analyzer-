...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        period_offsets = {
            ...
            "15년": pd.DateOffset(years=15),
        }

        for name, offset in period_offsets.items():
            target_date = last_date - offset
            target_data = close[close.index <= target_date]
            if not target_data.empty:
                past_close = float(target_data.iloc[-1])
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            st.markdown(f"**종가:** {last_close:.2f} USD ({last_date.strftime('%Y-%m-%d')})")
