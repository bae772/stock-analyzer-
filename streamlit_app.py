...
for idx, ticker in enumerate(tickers):
    ...
    try:
        data = yf.download(ticker, period="7d", interval="5m", progress=False, auto_adjust=True)
        ...
        try:
            # 1일
            one_day = last_date - BDay(1)
            if one_day in close.index:
                pct_changes['1일'] = round((last_close - close.loc[one_day])/close.loc[one_day]*100,2)
            # 1주
            one_week = last_date - BDay(5)
            if one_week in close.index:
                pct_changes['1주'] = round((last_close - close.loc[one_week])/close.loc[one_week]*100,2)
            # 3개월 (별도 일간 데이터)
            daily_data = yf.download(ticker, period="3mo", interval="1d", progress=False, auto_adjust=True)
            if not daily_data.empty:
                daily_close = daily_data['Close']
                three_months_date = daily_close.index[0]  # 3개월 전 첫 거래일
                pct_changes['3달'] = round((last_close - daily_close.loc[three_months_date])/daily_close.loc[three_months_date]*100,2)
        except Exception as e:
            st.warning(f"{ticker}: 기간별 퍼센트 계산 오류 ({e})")
        ...
        base_score += 0  # 실시간 시세 기준은 change_pct 대신 0 처리
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            st.markdown(f"**실시간 종가:** {last_close:.2f} USD ({last_date.strftime('%Y-%m-%d %H:%M')})")
            ...
            # ── 1일, 1주, 3개월 등락률
            st.markdown("**기간별 등락률:**")
            for period_name, pct in pct_changes.items():
                st.markdown(f"- {period_name} 전 대비: {pct}%")
