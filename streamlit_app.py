...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 정확한 영업일 기준 퍼센트 계산 (1일, 1주, 1개월, 3개월)
        pct_changes = {}
        lookbacks = {"1일":1, "1주":5, "1개월":22, "3개월":66}
        today = close.index[-1]

        for name, days in lookbacks.items():
            past_date = today - BDay(days)
            if past_date in close.index:
                past_close = close.loc[past_date]
            else:
                # 가장 가까운 이전 거래일
                past_close = close[:past_date].iloc[-1]
        ...
        base_score += max(pct_changes.values())*2.0*score_weights.get("BUY",1.0)  # 1일~3개월 중 최대 상승률 반영
