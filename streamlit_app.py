...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        change_pct = round((last_close - prev_close)/prev_close*100, 2)  # ← 오류 수정
        ...
        for name, days_back in period_dict.items():
            if len(close) > days_back:
                past_close = float(close.iloc[-days_back])
                pct = round((last_close - past_close)/past_close*100,2)
                if pct >= 0:
                    pct_changes[name] = f"{pct}% 상승"
                else:
                    pct_changes[name] = f"{abs(pct)}% 하락"
            else:
                pct_changes[name] = "데이터 부족"
