...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            # ── 기간별 수익률 ──
            period_return = round((close.iloc[-1]-close.iloc[0])/close.iloc[0]*100,2)
            st.markdown(f"**선택 기간({selected_time}) 수익률:** {period_return}% ({'상승' if period_return>=0 else '하락'})")

            # ── 3달이면 1달 단위 수익률 추가 ──
            if selected_time=="3달" and len(close)>=20:
                month_returns=[]
                for i in range(0,len(close),20):
                    end_idx = min(i+20,len(close))
                    start_price = close.iloc[i]
                    end_price = close.iloc[end_idx-1]
                    month_ret = round((end_price-start_price)/start_price*100,2)
                    month_returns.append(month_ret)
                for idx, ret in enumerate(month_returns):
                    st.markdown(f"**{idx+1}개월 차 수익률:** {ret}% ({'상승' if ret>=0 else '하락'})")
