...
# 사용자 입력
ticker = st.text_input("티커 입력 (예: BMR, TSLA, MARA) 또는 여러개 쉼표로 구분", value="BMR").upper()
avg_price = st.number_input("보유 평단가 입력 (없으면 0)", min_value=0.0, step=0.01, value=0.0)

# 기간 선택
period_option = st.selectbox(
    "데이터 기간 선택",
    ["15년", "10년", "5년", "1년", "3개월", "1개월", "1주", "1일", "실시간"]
)
...
for t in tickers:
    ...
    try:
        # 데이터 다운로드
        if period_option == "실시간":
            data = yf.download(t, period="1d", interval=interval, progress=False, auto_adjust=True)
        else:
            data = yf.download(t, period=period_map.get(period_option, "1y"), interval=interval, progress=False, auto_adjust=True)

        if data.empty or len(data) < 20:
            st.warning(f"{t}: 데이터가 부족하거나 티커를 찾을 수 없습니다.")
            continue
        ...
        <div style="background:#000; color:white; padding:30px; border-radius:20px; 
            ...
            <h1 style="color:#00ffcc; margin:0; font-size:4.5em;">{t}</h1>
        ...
        fig.update_layout(title=f"{t} 차트", xaxis_title="기간", yaxis_title="가격", template='plotly_dark')
        st.plotly_chart(fig, use_container_width=True)

        st.caption(f"업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M')} │ 데이터: Yahoo Finance")

    except Exception as e:
        st.error(f"{t} 오류 발생: {e}")
