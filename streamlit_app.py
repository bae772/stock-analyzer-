...
st.title("베짱이 계산기 (미국주식 AI 분석 + 투자 전략 + 매도 시점)")
...
if ticker:
    try:
        # ── 데이터 다운로드 ──
        data = yf.download(ticker, start=start_date, end=end_date + timedelta(days=1), progress=False, auto_adjust=True)
        if data.empty or len(data) < 10:
            st.warning("선택한 기간의 데이터가 부족합니다. 최근 1년 데이터로 불러옵니다.")
        ...
        if last_close > last_ma20 and last_ma20 > last_ma50:
            long_strategy = "장기: 상승추세, 비중 확대 가능, 장기 보유 추천"
        ...
        st.caption(f"업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M')} │ 데이터: Yahoo Finance (실시간 약간 지연)")
