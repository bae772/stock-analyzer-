...
# ── 거래일 기준 퍼센트 계산 함수 ──
def pct_change_from_trading_days(df_close, trading_days):
    if len(df_close) <= trading_days:
        return None
    past_price = float(df_close.iloc[-trading_days-1])
...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        for label, days in trading_days_map.items():
            pct = pct_change_from_trading_days(close, days)
            percent_changes[label] = f"{pct:+}%" if pct is not None else "데이터 부족"
        ...
        base_score += percent_changes[selected_time].replace("%","")*2.0 if percent_changes[selected_time]!="데이터 부족" else 0
