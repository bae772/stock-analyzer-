...
for idx, ticker in enumerate(tickers):
    ...
    try:
        ...
        # ── 기간별 등락률 계산 (날짜 기준 정확하게, 상승/하락 포함)
        pct_changes = {}
        period_offsets = {
            "1일": BDay(1),
            "1주": BDay(5),
            "1개월": DateOffset(months=1),
            "3달": DateOffset(months=3),
            "6달": DateOffset(months=6),
            "1년": DateOffset(years=1),
            "5년": DateOffset(years=5),
            "10년": DateOffset(years=10),
            "15년": DateOffset(years=15)
        }

        for period_name, offset in period_offsets.items():
            past_date = close.index[-1] - offset
            past_close_series = close[close.index <= past_date]
            if not past_close_series.empty:
                past_close = float(past_close_series.iloc[-1])
                pct = round((last_close - past_close)/past_close*100, 2)
                pct_text = f"{pct}% 상승" if pct >= 0 else f"{abs(pct)}% 하락"
                pct_changes[period_name] = pct_text
            else:
                pct_changes[period_name] = "데이터 부족"
        ...
        base_score += 0  # 이전 change_pct 제거, 대신 기간별 정확히 표시
        ...
        with st.expander(f"{ticker} 정보 보기", expanded=True):
            ...
            # ── 기간별 등락률 표시
            st.markdown("**기간별 등락률:**")
            for p_name, pct_text in pct_changes.items():
                st.markdown(f"- {p_name} 전 대비: {pct_text}")
